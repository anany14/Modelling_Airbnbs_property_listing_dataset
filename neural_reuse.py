...
class AirbnbBedroomDataset(Dataset):
    ...
    def __init__(self,df: pd.DataFrame = None) -> None:
        ...
        self.X,self.y = self.preprocess_data(df,label="bedrooms")
        print(self.X.head())
    ...
    def preprocess_data(self, df: pd.DataFrame, label: str) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        # Concatenate the encoded 'Category' column horizontally (along columns)
        X_preprocessed = pd.concat([X_scaled, pd.Series(category_encoded, name='Category')], axis=1)
...
class NN(torch.nn.Module):
    ...
    def __init__(self, config: dict) -> None:
        ...
        self.layers = torch.nn.Sequential(
            nn.Linear(12, self.hidden_layer_width),
            nn.ReLU(),
...
def save_model(model: NN, hyperparameters: dict, performance_metrics: dict, file_path: str = "models/neural_networks/regression/bedroom") -> None:
    """
